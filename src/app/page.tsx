...
interface Budget {
  id: string;
  category: string;
  period: string; // Period key like "2024-P3"
  limit: number;
}
...
interface Summary {
  ...
  periodSpending: Record<string, number>;
  budgets: Budget[];
...
export default function BudgetApp() {
  ...
  // Current period for budget tracking
  const [currentPeriod, setCurrentPeriod] = useState(() => {
    const period = getCurrentPeriod();
    return {
      id: period.id,
      year: period.year,
      key: getPeriodKey(period.id, period.year),
    };
  });
  ...
  const { data: budgets = [] } = useQuery<Budget[]>({
    queryKey: ['budgets', currentPeriod.key],
    queryFn: () => fetchData<Budget[]>(`/api/budgets?period=${currentPeriod.key}`),
  });
  ...
  // Summary query - depends on current period
  const { data: summary } = useQuery<Summary>({
    queryKey: ['summary', currentPeriod.key],
    queryFn: () => fetchData<Summary>(`/api/summary?period=${currentPeriod.key}`),
  });

  // Analysis query - for comprehensive analytics
  const { data: analysisData, isLoading: analysisLoading } = useQuery({
    queryKey: ['analysis', analysisPeriod, currentPeriod.key, comparePeriodKey],
    queryFn: async () => {
      let url = `/api/analysis?period=${analysisPeriod}`;
      if (comparePeriodKey) {
        url += `&currentPeriod=${currentPeriod.key}&compareWith=${comparePeriodKey}`;
      }
      const res = await fetch(url);
    ...
    <>
      ...
      <div className="min-h-screen bg-background flex justify-center">
        ...
        <header className="bg-gradient-to-br from-blue-600 to-blue-800 text-white p-5 rounded-b-3xl shadow-lg">
          ...
          {/* Period Info Bar */}
          <div className="flex justify-between items-center mb-3 bg-white/10 rounded-xl p-2 px-3">
            <div className="flex items-center gap-2">
              <span className="text-xs opacity-80">Period {currentPeriod.id} of 12</span>
              <span className="text-xs opacity-60">•</span>
              <span className="text-xs opacity-80">
                {getPeriodDates(currentPeriod.id, currentPeriod.year).startDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })} - {getPeriodDates(currentPeriod.id, currentPeriod.year).endDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
              </span>
            </div>
            <PeriodNavigation 
              selectedPeriod={currentPeriod} 
        ...
        <main className="flex-1 overflow-y-auto p-4 pb-24">
              ...
              <div className="bg-card rounded-2xl p-3 border shadow-sm">
                {summary?.budgets && summary.budgets.length > 0 ? (
                  summary.budgets.map((b) => {
                    const spent = summary.periodSpending?.[b.category] || 0;
          ...
          {view === 'budgets' && (
            <div className="animate-fadeIn">
              <div className="bg-blue-50 dark:bg-blue-950 rounded-2xl p-3 mb-4 border border-blue-200 dark:border-blue-800">
                <label className="text-xs font-medium text-blue-700 dark:text-blue-300 mb-1 block">Period:</label>
                <div className="text-lg font-bold text-blue-800 dark:text-blue-200">
                  {getPeriodDisplayName(currentPeriod.id, currentPeriod.year)}
                </div>
                <div className="text-xs text-blue-600 dark:text-blue-400 mt-1">
                  Period {currentPeriod.id} of 12
                </div>
              </div>

              <div className="bg-card rounded-2xl p-3 border shadow-sm mb-4">
                <h3 className="font-semibold mb-3">Current Period Budgets</h3>
                {summary?.budgets && summary.budgets.length > 0 ? (
                  summary.budgets.map((b) => {
                    const spent = summary.periodSpending?.[b.category] || 0;
                    const pct = Math.min((spent / b.limit) * 100, 100);
              ...
              <div className="bg-card rounded-2xl p-4 border shadow-sm">
                <h3 className="font-semibold mb-3">Set Budget for Period {currentPeriod.id}</h3>
                  ...
                  <button
                    onClick={() => {
                      if (budgetForm.category && budgetForm.limit) {
                        addBudget.mutate({ category: budgetForm.category, period: currentPeriod.key, limit: parseFloat(budgetForm.limit) });
                      }
                    }}
          ...
          {view === 'analysis' && (
            <div className="animate-fadeIn">
              ...
              <div className="bg-card rounded-2xl p-3 border shadow-sm mb-4">
                <div className="flex justify-between items-center">
                  <div className="text-sm font-medium">Compare Periods</div>
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-muted-foreground">vs</span>
                    <select
                      value={comparePeriodKey || ''}
                      onChange={(e) => setComparePeriodKey(e.target.value || null)}
                      className="p-1.5 rounded-lg bg-muted border outline-none text-xs"
                    >
                      <option value="">Select period...</option>
                      {analysisData?.allPeriods?.filter((p: { key: string }) => p.key !== currentPeriod.key).map((p: { key: string; displayName: string }) => (
                ...
                {comparePeriodKey && (
                  <div className="text-xs text-muted-foreground mt-1">
                    Comparing {getPeriodDisplayName(currentPeriod.id, currentPeriod.year)} with selected period
                  </div>
                )}
              </div>

              {/* Period Comparison Results */}
              {analysisData?.periodComparison && (
                <div className="bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-950 dark:to-purple-950 rounded-2xl p-4 border border-blue-200 dark:border-blue-800 mb-4">
                  <h3 className="font-semibold mb-3 text-blue-700 dark:text-blue-300">📊 Period Comparison</h3>
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-3">
                    <div className="bg-white/50 dark:bg-black/20 rounded-xl p-3">
                      ...
                      <div className="text-xs font-medium text-blue-600">{analysisData.periodComparison.currentPeriod.key}</div>
                    </div>
                    <div className="bg-white/50 dark:bg-black/20 rounded-xl p-3">
                      <div className="text-xs text-muted-foreground mb-1">Compare With</div>
                      <div className="text-xs font-medium text-purple-600">{analysisData.periodComparison.comparePeriod.key}</div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex justify-between items-center p-2 bg-white/50 dark:bg-black/20 rounded-lg">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="font-semibold text-green-600">{formatCurrency(analysisData.periodComparison.currentPeriod.income)}</span>
                        <span className={`text-xs px-2 py-0.5 rounded-full ${
                          analysisData.periodComparison.changes.income >= 0 
                            ? 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300' 
                            : 'bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-300'
                        }`}>
                          {analysisData.periodComparison.changes.income >= 0 ? '↑' : '↓'} {Math.abs(analysisData.periodComparison.changes.income).toFixed(1)}%
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="flex justify-between items-center p-2 bg-white/50 dark:bg-black/20 rounded-lg">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="font-semibold text-red-600">{formatCurrency(analysisData.periodComparison.currentPeriod.expenditure)}</span>
                        <span className={`text-xs px-2 py-0.5 rounded-full ${
                          analysisData.periodComparison.changes.expenditure <= 0 
                            ? 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300' 
                            : 'bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-300'
                        }`}>
                          {analysisData.periodComparison.changes.expenditure <= 0 ? '↓' : '↑'} {Math.abs(analysisData.periodComparison.changes.expenditure).toFixed(1)}%
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="flex justify-between items-center p-2 bg-white/50 dark:bg-black/20 rounded-lg">
                      ...
                      <div className="flex items-center gap-2">
                        <span className={`font-semibold ${analysisData.periodComparison.currentPeriod.netFlow >= 0 ? 'text-blue-600' : 'text-orange-600'}`}>
                          {formatCurrency(analysisData.periodComparison.currentPeriod.netFlow)}
                        </span>
                        <span className={`text-xs px-2 py-0.5 rounded-full ${
                          analysisData.periodComparison.changes.netFlow >= 0 
                            ? 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300' 
                            : 'bg-orange-100 text-orange-700 dark:bg-orange-900 dark:text-orange-300'
                        }`}>
                          {analysisData.periodComparison.changes.netFlow >= 0 ? '↑' : '↓'} {formatCurrency(Math.abs(analysisData.periodComparison.changes.netFlow))}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Category Comparison */}
                  {analysisData.periodComparison.categoryComparison && analysisData.periodComparison.categoryComparison.length > 0 && (
                    <div className="mt-3 pt-3 border-t border-blue-200 dark:border-blue-800">
                      <div className="text-xs font-medium text-muted-foreground mb-2">Category Changes</div>
                      <div className="space-y-1.5 max-h-40 overflow-y-auto">
                        {analysisData.periodComparison.categoryComparison.slice(0, 6).map((cat: { category: string; current: number; previous: number; change: number }) => (
                          <div key={cat.category} className="flex justify-between items-center text-xs">
                            <span className="truncate">{cat.category}</span>
                            <div className="flex items-center gap-2">
                              <span className="font-medium">{formatCurrency(cat.current, false)}</span>
              ...
              <div className="grid grid-cols-2 gap-3 mb-4">
                <div className="bg-green-50 dark:bg-green-950 rounded-2xl p-4 border border-green-200 dark:border-green-800">
                  ...
                  {analysisData?.comparison && !analysisData?.periodComparison && (
              ...
              {/* Period Trend */}
              {analysisData?.periodTrend && analysisData.periodTrend.length > 1 && (
                <div className="bg-card rounded-2xl p-4 border shadow-sm mb-4">
                  <h3 className="font-semibold mb-3">📅 Period Trend</h3>
                  <div className="flex items-end justify-between h-32 gap-1">
                    {analysisData.periodTrend.slice(-6).map((p: { periodKey: string; periodNumber: number; year: number; income: number; expenditure: number }) => {
                      const maxValue = Math.max(p.income, p.expenditure, 1);
                      const incomeHeight = (p.income / maxValue) * 100;
                      const expenseHeight = (p.expenditure / maxValue) * 100;
                      return (
                        <div key={p.periodKey} className="flex flex-col items-center flex-1 gap-1">
                          <div className="flex gap-0.5 h-24 w-full justify-center">
                            ...
                            <div
                              ...
                              title={`Expenditure: ${formatCurrency(p.expenditure)}`}
                            />
                          </div>
